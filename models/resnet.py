...
class ResNet_patch16(nn.Module):
    ...
    def forward(self, x, nce=False, val=False, BCL=False, fgssl=False):   
        B, _, _, _ = x.shape
        x = F.interpolate(x, size=(224*int(self.patch_num**0.5), 224*int(self.patch_num**0.5)))     
...
class ResNet_patch_overlap(nn.Module):
    # model_name, args.depth, args.num_classes, args.bottleneck, args.prete
    def __init__(self, dataset, depth, num_classes, bottleneck=False, pretext='None', patch_num=16, overlap=100):
        super(ResNet_patch_overlap, self).__init__()
        self.patch_num = patch_num 
    ...
    def forward(self, x, nce=False, val=False, BCL=False, fgssl=False):   
        ...
        n_grid  = int(self.patch_num**0.5)
        patch_size=224
        img_size = (patch_size*n_grid ) - self.overlap*(n_grid -1)
